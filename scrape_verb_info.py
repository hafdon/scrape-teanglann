...
def extract_verb_info(url):
    try:
        ...
        if verbal_noun_section:
            verbal_noun = verbal_noun_section.find_next(
        ...
        if verbal_adjective_section:
            verbal_adjective = verbal_adjective_section.find_next(
                "span", class_="value primary"
            ).text.strip()
...
output_cumulative_json = "output_cumulative.json"
existing_urls_txt = "existing_urls.txt"  # New file for URLs
...
# Update cumulative JSON file without duplicates
try:
    with open(output_cumulative_json, "r", encoding="utf-8") as f:
        existing_items = json.load(f)
except FileNotFoundError:
    existing_items = []

# Build a set of existing URLs for quick lookup
existing_urls = set(item["url"] for item in existing_items)

# Append new items that are not duplicates
for item in items_dict:
    if item["url"] not in existing_urls:
        existing_items.append(item)
        existing_urls.add(item["url"])

# Write the updated data back to the cumulative JSON file
with open(output_cumulative_json, "w", encoding="utf-8") as f:
    json.dump(existing_items, f, ensure_ascii=False, indent=4)

print(f"Cumulative data has been updated in {output_cumulative_json}.")

# Write the existing URLs to a text file for easy visual scanning
with open(existing_urls_txt, "w", encoding="utf-8") as f:
    for url in sorted(existing_urls):
        f.write(url + "\n")

print(f"Existing URLs have been written to {existing_urls_txt}.")
